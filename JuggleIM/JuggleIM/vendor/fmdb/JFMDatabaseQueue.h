...
/** To perform queries and updates on multiple threads, you'll want to use @c JFMDatabaseQueue .

 Using a single instance of @c JFMDatabase from multiple threads at once is a bad idea.  It has always been OK to make a @c JFMDatabase  object *per thread*.  Just don't share a single instance across threads, and definitely not across multiple threads at the same time.

 Instead, use @c JFMDatabaseQueue . Here's how to use it:

 First, make your queue.

@code
JFMDatabaseQueue *queue = [JFMDatabaseQueue databaseQueueWithPath:aPath];
@endcode

 Then use it like so:

@code
[queue inDatabase:^(JFMDatabase *db) {
    [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:1]];
    [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:2]];
    [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:3]];

    JFMResultSet *rs = [db executeQuery:@"select * from foo"];
    while ([rs next]) {
        //…
    }
}];
...
@code
[queue inTransaction:^(JFMDatabase *db, BOOL *rollback) {
    [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:1]];
    [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:2]];
    [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:3]];
...
@endcode
 ...
 @c JFMDatabaseQueue will run the blocks on a serialized queue (hence the name of the class).  So if you call @c JFMDatabaseQueue 's methods from multiple threads at the same time, they will be executed in the order they are received.  This way queries and updates won't step on each other's toes, and every one is happy.

 @warning Do not instantiate a single @c JFMDatabase  object and use it across multiple threads. Use @c JFMDatabaseQueue  instead.
 
 @warning The calls to @c JFMDatabaseQueue 's methods are blocking.  So even though you are passing along blocks, they will **not** be run on another thread.

 @sa JFMDatabase

 */

@interface JFMDatabaseQueue : NSObject
...
/** Create queue using path.
 ...
 @return The @c JFMDatabaseQueue  object. @c nil  on error.
 */
