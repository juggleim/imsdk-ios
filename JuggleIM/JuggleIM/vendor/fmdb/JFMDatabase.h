...
#import "JFMResultSet.h"
#import "JFMDatabasePool.h"
...
/** A SQLite ([https://sqlite.org/](https://sqlite.org/)) Objective-C wrapper.
 ...
 The three main classes in JFMDB are:

 - @c JFMDatabase - Represents a single SQLite database.  Used for executing SQL statements.

 - @c JFMResultSet - Represents the results of executing a query on an @c JFMDatabase .

 - @c JFMDatabaseQueue - If you want to perform queries and updates on multiple threads, you'll want to use this class.

 See also

 - @c JFMDatabasePool - A pool of @c JFMDatabase objects

 - @c FMStatement - A wrapper for @c sqlite_stmt

 External links

 - [JFMDB on GitHub](https://github.com/ccgus/fmdb) including introductory documentation
 - [SQLite web site](https://sqlite.org/)
 - [JFMDB mailing list](http://groups.google.com/group/fmdb)
 - [SQLite FAQ](https://sqlite.org/faq.html)

 @warning Do not instantiate a single @c JFMDatabase  object and use it across multiple threads. Instead, use @c JFMDatabaseQueue .

 */

@interface JFMDatabase : NSObject
...
/** Create a @c JFMDatabase  object.
 
 An @c JFMDatabase  is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.

 2. An zero-length string.  An empty database is created at a temporary location.  This database is deleted with the @c JFMDatabase  connection is closed.

 3. @c nil .  An in-memory database is created.  This database will be destroyed with the @c JFMDatabase  connection is closed.
...
JFMDatabase *db = [JFMDatabase databaseWithPath:fileURL.path];
@endcode
 ...
 @return @c JFMDatabase  object if successful; @c nil  if failure.
...
/** Create a @c JFMDatabase  object.
 ...
 2. @c nil .  An in-memory database is created.  This database will be destroyed with the @c JFMDatabase  connection is closed.
...
NSURL *folder  = [[NSFileManager defaultManager] URLForDirectory:NSApplicationSupportDirectory inDomain:NSUserDomainMask appropriateForURL:nil create:true error:&error];
NSURL *fileURL = [folder URLByAppendingPathComponent:@"test.db"];
JFMDatabase *db = [JFMDatabase databaseWithURL:fileURL];
@endcode
...
/** Initialize a @c JFMDatabase  object.
 
 An @c JFMDatabase  is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.

 2. A zero-length string.  An empty database is created at a temporary location.  This database is deleted with the @c JFMDatabase  connection is closed.
 ...
 NSURL *folder  = [[NSFileManager defaultManager] URLForDirectory:NSApplicationSupportDirectory inDomain:NSUserDomainMask appropriateForURL:nil create:true error:&error];
 NSURL *fileURL = [folder URLByAppendingPathComponent:@"test.db"];
 JFMDatabase *db = [[JFMDatabase alloc] initWithPath:fileURL.path];
 @endcode
...
/** Initialize a @c JFMDatabase  object.
 ...
 An @c JFMDatabase  is created with a local file URL to a SQLite database file.  This path can be one of these three:
 ...
 NSURL *folder  = [[NSFileManager defaultManager] URLForDirectory:NSApplicationSupportDirectory inDomain:NSUserDomainMask appropriateForURL:nil create:true error:&error];
 NSURL *fileURL = [folder URLByAppendingPathComponent:@"test.db"];
 JFMDatabase *db = [[JFMDatabase alloc] initWithURL:fileURL];
 @endcode
...
/** Execute select statement
 ...
 Executing queries returns an @c JFMResultSet  object if successful, and @c nil  upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the @c lastErrorMessage  and @c lastErrorMessage  methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[JFMResultSet next]>`) from one record to the other.
 ...
 @return A @c JFMResultSet  for the result set upon success; @c nil  upon failure. If failed, you can call @c lastError , @c lastErrorCode , or @c lastErrorMessage  for diagnostic information regarding the failure.
 
 @see JFMResultSet
 @see [`JFMResultSet next`](<[JFMResultSet next]>)
 @see [`sqlite3_bind`](https://sqlite.org/c3ref/bind_blob.html)
...
- (JFMResultSet * _Nullable)executeQuery:(NSString*)sql, ...;
...
- (JFMResultSet * _Nullable)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (JFMResultSet * _Nullable)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
/** Execute select statement
 ...
 `func executeQuery(sql: String, values: [Any]?) throws  -> JFMResultSet!`
...
- (JFMResultSet * _Nullable)executeQuery:(NSString *)sql values:(NSArray * _Nullable)values error:(NSError * _Nullable __autoreleasing *)error;
...
- (JFMResultSet * _Nullable)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary * _Nullable)arguments;


// Documentation forthcoming.
- (JFMResultSet * _Nullable)executeQuery:(NSString *)sql withVAList:(va_list)args;

/// Prepare SQL statement.
///
/// @param sql SQL statement to prepare, generally with `?` placeholders.

- (JFMResultSet *)prepare:(NSString *)sql;
...
/// The JFMDB version number as a string in the form of @c "2.7.8" .
///
/// If you want to compare version number strings, you can use NSNumericSearch option:
///
/// @code
/// NSComparisonResult result = [[JFMDatabase FMDBUserVersion] compare:@"2.11.0" options:NSNumericSearch];
/// @endcode
///
...
/** The JFMDB version
 ...
 This returns the JFMDB as hexadecimal value, e.g., @c 0x0243 for version 2.4.3.
...
JFMResultSet *rs = [db executeQuery:@"SELECT * FROM employees WHERE RemoveDiacritics(first_name) LIKE 'jose'"];
...
@code
myDB.dateFormat = [JFMDatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
@endcode
 ...
 @warning Note that @c NSDateFormatter  is not thread-safe, so the formatter generated by this method should be assigned to only one JFMDB instance and should not be used for other purposes.
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using @c JFMDatabase:storeableDateFormat .
 ...
 @warning Note there is no direct getter for the @c NSDateFormatter , and you should not use the formatter you pass to JFMDB for other purposes, as @c NSDateFormatter  is not thread-safe.
 */
...
/** Objective-C wrapper for @c sqlite3_stmt
 ...
 This is a wrapper for a SQLite @c sqlite3_stmt . Generally when using JFMDB you will not need to interact directly with `FMStatement`, but rather with @c JFMDatabase  and @c JFMResultSet  only.
