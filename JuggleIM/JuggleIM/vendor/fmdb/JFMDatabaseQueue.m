...
/*
 ...
 JFMDatabaseQueue is released on another thread and we're in the middle of doing
 something in dispatch_sync
 
 */

/*
 * A key used to associate the JFMDatabaseQueue object with the dispatch_queue_t it uses.
 * This in turn is used for deadlock detection by seeing if inDatabase: is called on
 * the queue's dispatch queue, which should not happen and causes a deadlock.
 */
static const void * const kDispatchQueueSpecificKey = &kDispatchQueueSpecificKey;

@interface JFMDatabaseQueue () {
    dispatch_queue_t    _queue;
    JFMDatabase          *_db;
}
@end

@implementation JFMDatabaseQueue

+ (instancetype)databaseQueueWithPath:(NSString *)aPath {
    JFMDatabaseQueue *q = [[self alloc] initWithPath:aPath];
...
+ (instancetype)databaseQueueWithPath:(NSString *)aPath flags:(int)openFlags {
    JFMDatabaseQueue *q = [[self alloc] initWithPath:aPath flags:openFlags];
...
#endif
        if (!success) {
            NSLog(@"JFMDatabaseQueue could not reopen database for path %@", _path);
            FMDBRelease(_db);
            _db  = 0x00;
            return 0x00;
        }
...
- (void)inDatabase:(__attribute__((noescape)) void (^)(JFMDatabase *db))block {
    ...
    JFMDatabaseQueue *currentSyncQueue = (__bridge id)dispatch_get_specific(kDispatchQueueSpecificKey);
    ...
    dispatch_sync(_queue, ^() {
        ...
        JFMDatabase *db = [self database];
        
        block(db);
        
        if ([db hasOpenResultSets]) {
            NSLog(@"Warning: there is at least one open result set around after performing [JFMDatabaseQueue inDatabase:]");
...
- (void)beginTransaction:(FMDBTransaction)transaction withBlock:(void (^)(JFMDatabase *db, BOOL *rollback))block {
    FMDBRetain(self);
    dispatch_sync(_queue, ^() { 
...
- (void)inImmediateTransaction:(__attribute__((noescape)) void (^)(JFMDatabase * _Nonnull, BOOL * _Nonnull))block {
    [self beginTransaction:FMDBTransactionImmediate withBlock:block];
}

- (NSError*)inSavePoint:(__attribute__((noescape)) void (^)(JFMDatabase *db, BOOL *rollback))block {
