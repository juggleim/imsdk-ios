...
- (BOOL)event:(NSInteger)event userInfo:(NSDictionary *)userInfo {
    ...
    switch (event) {
        ...
        case JConnEventConnectingTimeOut:
            JLogI(@"CON-Connect", @"websocket fail or timeout, clientIP is %@, osVersion is %@, networkId is %@, ispNum is %@, sdkVersion is %@, osVersion is %@", [JUtility getClientIP], [JUtility currentSystemVersion], [JUtility currentNetWork], [JUtility currentCarrier], JIMVersion, [JUtility currentSystemVersion]);
