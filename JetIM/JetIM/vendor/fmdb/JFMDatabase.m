...
// MARK: - JFMDatabase Private Extension

NS_ASSUME_NONNULL_BEGIN

@interface JFMDatabase () {
    void*               _db;
...
- (JFMResultSet * _Nullable)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray * _Nullable)arrayArgs orDictionary:(NSDictionary * _Nullable)dictionaryArgs orVAList:(va_list)args shouldBind:(BOOL)shouldBind;
...
// MARK: - JFMResultSet Private Extension

@interface JFMResultSet ()

- (int)internalStepWithError:(NSError * _Nullable __autoreleasing *)outErr;
+ (instancetype)resultSetWithStatement:(FMStatement *)statement usingParentDatabase:(JFMDatabase*)aDB shouldAutoClose:(BOOL)shouldAutoClose;
...
#pragma mark JFMDatabase instantiation and deallocation
...
static int FMDBDatabaseBusyHandler(void *f, int count) {
    JFMDatabase *self = (__bridge JFMDatabase*)f;
...
// but for folks who don't bother noticing that the interface to JFMDatabase changed,
// we'll still implement the method so they don't get suprise crashes
- (int)busyRetryTimeout {
    NSLog(@"%s:%d", __FUNCTION__, __LINE__);
    NSLog(@"JFMDB: busyRetryTimeout no longer works, please use maxBusyRetryTimeInterval");
    return -1;
}
...
#pragma unused(i)
    NSLog(@"%s:%d", __FUNCTION__, __LINE__);
    NSLog(@"JFMDB: setBusyRetryTimeout does nothing, please use setMaxBusyRetryTimeInterval:");
}
...
- (void)closeOpenResultSets {
    ...
    for (NSValue *rsInWrappedInATastyValueMeal in openSetCopy) {
        JFMResultSet *rs = (JFMResultSet *)[rsInWrappedInATastyValueMeal pointerValue];
...
- (void)resultSetDidClose:(JFMResultSet *)resultSet {
...
- (void)setCachedStatement:(FMStatement*)statement forQuery:(NSString*)query {
    ...
    if (!query) {
        NSLog(@"API misuse, -[JFMDatabase setCachedStatement:forQuery:] query must not be nil");
        return;
    }
...
- (BOOL)goodConnection {
    ...
    JFMResultSet *rs = [self executeQuery:@"PRAGMA cipher_version"];
...
#else
    JFMResultSet *rs = [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (void)warnInUse {
    NSLog(@"The JFMDatabase %@ is currently in use.", self);
    
#ifndef NS_BLOCK_ASSERTIONS
    if (_crashOnErrors) {
        NSAssert(false, @"The JFMDatabase %@ is currently in use.", self);
        abort();
    }
#endif
}
...
- (BOOL)databaseExists {
    ...
    if (!_isOpen) {
        ...
        NSLog(@"The JFMDatabase %@ is not open.", self);
        
#ifndef NS_BLOCK_ASSERTIONS
        if (_crashOnErrors) {
            NSAssert(false, @"The JFMDatabase %@ is not open.", self);
            abort();
        }
#endif
...
- (NSError*)errorWithMessage:(NSString *)message {
    ...
    return [NSError errorWithDomain:@"JFMDatabase" code:sqlite3_errcode(_db) userInfo:errorMessage];
}
...
- (JFMResultSet *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments {
    return [self executeQuery:sql withArgumentsInArray:nil orDictionary:arguments orVAList:nil shouldBind:true];
}

- (JFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args shouldBind:(BOOL)shouldBind {
    ...
    rs = [JFMResultSet resultSetWithStatement:statement usingParentDatabase:self shouldAutoClose:shouldBind];
    [rs setQuery:sql];
...
- (JFMResultSet *)executeQuery:(NSString*)sql, ... {
    va_list args;
    va_start(args, sql);
...
- (JFMResultSet *)executeQueryWithFormat:(NSString*)format, ... {
    va_list args;
...
- (JFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments {
    return [self executeQuery:sql withArgumentsInArray:arguments orDictionary:nil orVAList:nil shouldBind:true];
}

- (JFMResultSet *)executeQuery:(NSString *)sql values:(NSArray *)values error:(NSError * __autoreleasing *)error {
    JFMResultSet *rs = [self executeQuery:sql withArgumentsInArray:values orDictionary:nil orVAList:nil shouldBind:true];
    if (!rs && error) {
...
- (JFMResultSet *)executeQuery:(NSString*)sql withVAList:(va_list)args {
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError * _Nullable __autoreleasing *)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    JFMResultSet *rs = [self executeQuery:sql withArgumentsInArray:arrayArgs orDictionary:dictionaryArgs orVAList:args shouldBind:true];
    if (!rs) {
        if (outErr) {
...
#else
    NSString *errorMessage = NSLocalizedStringFromTable(@"Save point functions require SQLite 3.7", @"JFMDB", nil);
...
- (NSError*)inSavePoint:(__attribute__((noescape)) void (^)(BOOL *rollback))block {
    ...
    NSString *errorMessage = NSLocalizedStringFromTable(@"Save point functions require SQLite 3.7", @"JFMDB", nil);
    if (self.logsErrors) NSLog(@"%@", errorMessage);
    return [NSError errorWithDomain:@"JFMDatabase" code:0 userInfo:@{NSLocalizedDescriptionKey : errorMessage}];
#endif
}
