...
- (void)pushDatabaseBackInPool:(JFMDatabase*)db;
- (JFMDatabase*)db;

@end


@implementation JFMDatabasePool
...
- (void)pushDatabaseBackInPool:(JFMDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if ([self->_databaseInPool containsObject:db]) {
            [[NSException exceptionWithName:@"Database already in pool" reason:@"The JFMDatabase being put back into the pool is already present in the pool" userInfo:nil] raise];
        }
