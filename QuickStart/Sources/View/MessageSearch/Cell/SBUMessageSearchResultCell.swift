...
open class SBUMessageSearchResultCell: SBUTableViewCell {
    ...
    open func configure(message: JMessage) {
        ...
        if let senderId = self.message?.senderUserId {
            if let profileURL = JIM.shared().userInfoManager.getUserInfo(senderId).portrait {
            ...
            if let userName = JIM.shared().userInfoManager.getUserInfo(senderId).userName {
