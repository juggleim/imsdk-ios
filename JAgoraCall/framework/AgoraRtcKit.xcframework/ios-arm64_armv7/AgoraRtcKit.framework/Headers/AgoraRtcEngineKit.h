 ...
 * The SDK triggers the \ref AgoraRtcEngineDelegate.rtcEngine:renewTokenResult:code: callback after the token is renewed.
 *
...
/**
 * @brief Destroys the AgoraRtcEngineKit object.
 *
 * @details Releases AgoraRtcEngineKit instance and releases its associated resources.
...
/**
 * @since v4.6.0
 * @brief Destroys the RtcEngine object.
 *
 * @details Releases RtcEngine instance and releases its associated resources.
 * It is recommended for applications where users make occasional voice or video calls.
 * When the real-time communication functionality is no longer needed,
 * call this method to free up resources for other operations.
 * Once the application has called this method to destroy the created
 ...
 * new AgoraRtcEngineKit instance.
 *
 * @note Do not call this method from within any SDK callback, as this may result in a deadlock.
 *
 * @param engineReleasedBlock A callback block when AgoraRtcEngineKit has been destroyed,
 * used to configure synchronous or asynchronous destruction of the AgoraRtcEngineKit:
 * - `non-nil`: Asynchronous destruction. The method returns immediately,
 * and the engine resources may not be fully released yet. The [engineReleasedBlock](void (^)(void))
 * will be triggered once the engine destruction is complete.
 * - `nil`: Synchronous destruction. The method returns only after the engine resources
 * have been fully released, which is the same as [destroy]([AgoraRtcEngineKit destroy:]).
 */
 ...
 * @param sourceType The media source type, e.g., AgoraMediaSourceTypePrimaryCamera. See AgoraMediaSourceType.
 *
 * @return
 * - The pointer to AgoraVideoEffectObject, if the method call succeeds.
...
/**
 * @brief Destroys a video effect object instance.
 *
 * @since v4.6.0
 *
 * @param videoEffectObject The pointer to the AgoraVideoEffectObject instance to be destroyed.
 *
 * @return
 * - 0: Success.
 * - < 0: Failure.
 ...
 * - 1: Mono.
 * - 2: Stereo.
* @param samplesPerCall Sampling points in the called data
 * returned in `onPlaybackAudioFrameBeforeMixing`. For example, it is usually set as 1024 for
 * stream pushing.
 * @return int
 * - 0: Success.
 * - < 0: Failure.
